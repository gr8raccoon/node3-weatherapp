const request = require('request')

// const geocode = (address, callback) => {
//     const url = 'https://api.mapbox.com/geocoding/v5/mapbox.places/' + encodeURIComponent(address) + '.json?access_token=pk.eyJ1IjoiZnJvc3R5dmxhZCIsImEiOiJjanp3cDBkdHQwMGFxM2xtbTZ0MG1pdnF3In0.McgXmwa-hm_nLbtpdwvsrw'

//     request({ url, json: true }, (error,{ body }) => {
//         if(error) {
//             callback('Unable to connect to geolocation services', undefined)
//         } else if (body.features.length === 0) {
//             callback('No matching results', undefined)
//         } else {
//             const latitude = body.features[0].geometry.coordinates[1]
//             const longitude = body.features[0].geometry.coordinates[0]
//             const location = body.features[0].place_name
//             callback(undefined, {
//                 latitude,
//                 longitude,
//                 location
//             })
//         }
//     })
// }

const geocode = (address, callback) => {
    const url = 'https://api.mapbox.com/geocoding/v5/mapbox.places/' + encodeURIComponent(address) + '.json?access_token=pk.eyJ1IjoiZnJvc3R5dmxhZCIsImEiOiJjanp3cDBkdHQwMGFxM2xtbTZ0MG1pdnF3In0.McgXmwa-hm_nLbtpdwvsrw'

    request({ url, json: true }, (error, { body }) => {
        if (error) {
            callback('Unable to connect to location services!', undefined)
        } else if (body.features.length === 0) {
            callback('Unable to find location. Try another search.', undefined)
        } else {
            callback(undefined, {
                latitude: body.features[0].center[1],
                longitude: body.features[0].center[0],
                location: body.features[0].place_name
            })
        }
    })
}

module.exports = geocode